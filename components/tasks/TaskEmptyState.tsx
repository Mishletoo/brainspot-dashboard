...
export default function TaskEmptyState({ noServices, onAdd }: Props) {
  if (noServices) {
    return (
      <div className="flex flex-1 items-center justify-center rounded-2xl border border-dashed border-zinc-700 bg-[#1c212b]">
        <div className="text-center">
          ...
          <p className="mt-1 text-xs text-zinc-600">
            Tasks belong to services. Add a service first.
          </p>
          <Link
            href="/services"
            className="mt-4 inline-block rounded-xl bg-lime-400 px-4 py-2 text-sm font-semibold text-zinc-900 transition hover:bg-lime-300"
          >
            Go to Services
          </Link>
        </div>
